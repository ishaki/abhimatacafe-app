...
import { Settings, Save, RefreshCw, Database, Wifi, Bell, Shield, Coffee, Info, QrCode, Download, Eye, AlertTriangle, Trash2 } from 'lucide-react'
...
const SettingsPage = () => {
  ...
  const [resetTxModalOpen, setResetTxModalOpen] = useState(false)
  const [resetTxConfirmText, setResetTxConfirmText] = useState('')
  const [resetTxBusy, setResetTxBusy] = useState(false)
  ...
  const openResetTransactionsModal = () => {
    setResetTxConfirmText('')
    setResetTxModalOpen(true)
  }

  const closeResetTransactionsModal = () => {
    if (resetTxBusy) return
    setResetTxModalOpen(false)
    setResetTxConfirmText('')
  }

  const handleResetTransactions = async () => {
    if (resetTxConfirmText !== 'RESET') return
    setResetTxBusy(true)
    try {
      const resp = await api.post('/settings/reset-transactions', {
        confirm: 'RESET',
      })
      const deleted = resp.data?.deleted || {}
      const summary = `${deleted.orders || 0} orders, ${deleted.expenses || 0} expenses cleared`
      toast.success(summary)
      setResetTxModalOpen(false)
      setResetTxConfirmText('')
    } catch (error) {
      toast.error(
        error.response?.data?.error || 'Failed to reset transactional data'
      )
    } finally {
      setResetTxBusy(false)
    }
  }
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="border-t border-gray-200 mt-4 pt-4">
                ...
                <button
                  onClick={openResetTransactionsModal}
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Clears all orders, expenses, and daily counters. Users, menu, and settings are kept.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-60">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="flex items-start gap-3 px-6 py-4 border-b border-gray-200">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900">Reset Transactions</h3>
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <ul className="text-sm text-gray-700 list-disc list-inside space-y-1 ml-2">
                  ...
                  <li>All expenses</li>
                  <li>All daily takeaway counters</li>
                  <li>All customer ordering sessions</li>
                </ul>
              </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                ...
                <p className="text-xs text-green-700">
                  Users, menu items, settings, audit logs, active login sessions
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={resetTxConfirmText}
                  onChange={(e) => setResetTxConfirmText(e.target.value)}
                  disabled={resetTxBusy}
                  autoFocus
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex gap-3">
              <button
                onClick={closeResetTransactionsModal}
                disabled={resetTxBusy}
              ...
              <button
                ...
                disabled={resetTxBusy || resetTxConfirmText !== 'RESET'}
              ...
              >
                {resetTxBusy ? 'Resetting...' : 'Reset Transactions'}
              </button>
            </div>
          </div>
        </div>
